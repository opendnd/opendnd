...
// Ontology: https://docs.opendnd.org/ours/ontology.json (version 0.1.0)
...
/** Feat type: Categories of feat in the 2024 rules. */
export const featTypeCodes = [
  "origin",
  "general",
  "fighting-style",
  "epic-boon",
] as const;
export const featTypeSchema = z.enum(featTypeCodes);
...
/** Participant role: How an actor took part in an event. */
...
export const participantRoleSchema = z.enum(participantRoleCodes);
...
/** Poison type: How a poison is delivered. */
export const poisonTypeCodes = [
  "contact",
  "ingested",
  "inhaled",
  "injury",
] as const;
export const poisonTypeSchema = z.enum(poisonTypeCodes);
...
/** Relationship fact type: Dated facts recorded on a relationship (GEDCOM X fact types, extended for oaths and homage). */
export const relationshipFactTypeCodes = [
  "birth",
  "marriage",
  "betrothal",
  "divorce",
...
export const relationshipFactTypeSchema = z.enum(relationshipFactTypeCodes);
export type RelationshipFactType = z.infer<typeof relationshipFactTypeSchema>;

/** Relationship type: Kinds of relationship between two people (GEDCOM X types plus feudal and social ties). The parties may be people, factions or places; the asymmetric types name the senior party first. */
...
/** A quadtree cell token: the cell id in hex with trailing zero nibbles removed. Containment is a prefix relationship. See ADR-006. */
export const cellSchema = z.string().regex(new RegExp("^[0-9a-f]{1,16}$"));
...
/** A choice the rules offer: pick `choose` from `from`, from everything in a category, or between nested choices. One of those three must be present. Options may themselves be choices, which is how the rules nest them. */
const choiceRawSchema = z.strictObject({
  choose: z.int().min(1),
  /** The wording of the choice, where it matters. */
  note: z.string().optional(),
  /** The things that may be chosen. */
  from: z.array(referenceSchema).optional(),
  /** A category to choose from when the list is open-ended, named as a model id optionally qualified by a code, e.g. `skill` or `item:martial-weapon`. */
  fromCategory: z.string().optional(),
  /** Nested choices, for a choice between choices. */
  get options() {
    return z.array(choiceRawSchema).optional();
  },
});
export const choiceSchema = choiceRawSchema.refine((value) => [["from"],["fromCategory"],["options"]].some((keys) => keys.every((key) => (value as Record<string, unknown>)[key] !== undefined)), { error: "one of from, or fromCategory, or options is required" });
...
export const provenanceSchema = z.strictObject({
  ...
  /** The published work this record's content is taken from and the licence it is used under. Content that ships in a module carries this so attribution survives export, override and re-publication. */
  source: z.strictObject({
    /** Title of the source document, e.g. a system reference document. */
    document: z.string().optional(),
    publisher: z.string().optional(),
    /** SPDX identifier where one exists, e.g. CC-BY-4.0; otherwise the licence name. */
    license: z.string().optional(),
    /** Where the source or its licence is published. */
    url: z.url().optional(),
...
export const temporalPositionSchema = z.strictObject({
  ...
  /** A named position when no numeric one is known, e.g. 'the Age of Ash'. A position carries a year, a nominal name, or both. */
  nominal: z.string().optional(),
}).refine((value) => [["year"],["nominal"]].some((keys) => keys.every((key) => (value as Record<string, unknown>)[key] !== undefined)), { error: "one of year, or nominal is required" });
...
export const characterSheetSchema = z.strictObject({
  ...
  abilityScores: z.strictObject({
    strength: z.int().min(1).max(30).optional(),
    dexterity: z.int().min(1).max(30).optional(),
    constitution: z.int().min(1).max(30).optional(),
    intelligence: z.int().min(1).max(30).optional(),
    wisdom: z.int().min(1).max(30).optional(),
    charisma: z.int().min(1).max(30).optional(),
  }).optional(),
  /** Hit points as they stand. */
  hitPoints: z.strictObject({
    current: z.int().min(0),
    maximum: z.int().min(0),
    temporary: z.int().min(0).optional(),
  }).optional(),
...
export const classSchema = z.strictObject({
  ...
  spellcasting: z.strictObject({
    ...
    /** How the class casts, in the rules' own sections. */
    info: z.array(z.strictObject({
      name: z.string(),
      description: z.string().optional(),
    })).optional(),
  }).optional(),
...
/** A thing that can be owned, carried or used. One model for equipment, magic items and poisons alike, because the rules treat them the same way and only the fields that apply are filled in. The shape follows the 5e SRD equipment tree so content written for it converts without reshaping; the field names are camelCase to match the rest of this ontology, which is a mechanical transform of theirs. A record may describe a kind of thing or one particular thing: an instance names its kind in `instanceOf`, and any field set on the instance overrides the kind's. */
export const itemSchema = z.strictObject({
  ...
  /** Whether it must be attuned to before it works. */
...
/** A tie between two parties, with dated facts (GEDCOM X Relationship extended for succession and for parties that are not people). Either party may be a person, a faction or a place, so the same record carries a marriage, a house's vassalage to a crown, or a person's membership of a guild. */
...
export const speciesSchema = z.strictObject({
  ...
  /** Gene key to the phenotype it expresses. Common genes are '<category>:C<chromosome>:<dominant roll>'; rare genes are '<category>:C<chromosome>:<rollA>=<rollB>'. */
...
/** A spell. The shape follows the 5e SRD 2024 spell records as published so far (pull request 1178): a single description and higher-level text, damage as a type with dice by slot level, and no subclass lists. The saving throw and area of effect are kept beyond what those records carry, because every spell's rules text has them and a character sheet needs them. */
export const spellSchema = z.strictObject({
  ...
  /** What it deals, and how that scales. */
  damage: z.strictObject({
    damageType: damageTypeSchema.optional(),
  ...
  savingThrow: z.strictObject({
    ability: abilitySchema.optional(),
    /** What happens on a successful save: none, half. */
...
export const worldSchema = z.strictObject({
  ...
  map: z.strictObject({
    ...
    maxZoom: z.int().min(0).max(24).optional(),
...
export const vocabularies = {
  ...
  "feat-type": {
    id: "feat-type",
    name: "Feat type",
    description: "Categories of feat in the 2024 rules.",
    codes: [
      { code: "origin", display: "Origin" },
      { code: "general", display: "General" },
      { code: "fighting-style", display: "Fighting style" },
      { code: "epic-boon", display: "Epic boon" },
    ],
  },
  "feature-source": {
    id: "feature-source",
  ...
  "participant-role": {
    ...
    description: "How an actor took part in an event.",
    codes: [
      { code: "attacker", display: "Attacker" },
      { code: "besieged", display: "Besieged" },
      { code: "besieger", display: "Besieger" },
      { code: "child", display: "Child" },
      { code: "claimant", display: "Claimant" },
      { code: "deceased", display: "Deceased" },
      { code: "defeated", display: "Defeated" },
      { code: "defender", display: "Defender" },
      { code: "deposed", display: "Deposed" },
      { code: "envoy", display: "Envoy" },
      { code: "father", display: "Father" },
      { code: "founder", display: "Founder" },
      { code: "guest", display: "Guest" },
      { code: "heir", display: "Heir" },
      { code: "holder", display: "Holder" },
      { code: "host", display: "Host" },
      { code: "leader", display: "Leader" },
      { code: "mother", display: "Mother" },
      { code: "participant", display: "Participant" },
      { code: "perpetrator", display: "Perpetrator" },
      { code: "predecessor", display: "Predecessor" },
      { code: "signatory", display: "Signatory" },
      { code: "spouse", display: "Spouse" },
      { code: "successor", display: "Successor" },
      { code: "victim", display: "Victim" },
      { code: "victor", display: "Victor" },
      { code: "widowed", display: "Widowed" },
      { code: "witness", display: "Witness" },
    ],
  },
  "person-status": {
    id: "person-status",
  ...
  "poison-type": {
    id: "poison-type",
    name: "Poison type",
    description: "How a poison is delivered.",
    codes: [
      { code: "contact", display: "Contact" },
      { code: "ingested", display: "Ingested" },
      { code: "inhaled", display: "Inhaled" },
      { code: "injury", display: "Injury" },
    ],
  },
  precision: {
    id: "precision",
  ...
  "relationship-fact-type": {
    id: "relationship-fact-type",
    name: "Relationship fact type",
    description: "Dated facts recorded on a relationship (GEDCOM X fact types, extended for oaths and homage).",
    codes: [
      { code: "birth", display: "Birth" },
      { code: "marriage", display: "Marriage" },
      { code: "betrothal", display: "Betrothal" },
      { code: "divorce", display: "Divorce" },
      { code: "annulment", display: "Annulment" },
      { code: "separation", display: "Separation" },
      { code: "adoption", display: "Adoption" },
      { code: "fostering", display: "Fostering" },
      { code: "oath", display: "Oath" },
      { code: "homage", display: "Homage" },
      { code: "pact", display: "Pact" },
      { code: "betrayal", display: "Betrayal" },
      { code: "reconciliation", display: "Reconciliation" },
      { code: "estrangement", display: "Estrangement" },
    ],
  },
  "relationship-type": {
    id: "relationship-type",
    name: "Relationship type",
    description: "Kinds of relationship between two people (GEDCOM X types plus feudal and social ties). The parties may be people, factions or places; the asymmetric types name the senior party first.",
    codes: [
      ...
      { code: "member-of", display: "Member of" },
      { code: "resident-of", display: "Resident of" },
    ],
  },
  resource: {
    id: "resource",
