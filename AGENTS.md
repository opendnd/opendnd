...
- A model manifest's `relationships` name `Reference`-typed properties of its schema, dotted for nested ones; the validator refuses anything else. A model whose in-world span lives in its own fields declares them in `x-ours-valid-time`.
