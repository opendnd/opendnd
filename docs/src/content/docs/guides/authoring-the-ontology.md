...
2. Write `models/<id>.json`: the OURS `Model` with `name`, `description`, `category` (the group it is listed with: `play`, `people`, `places`, `lore`, `rules`, `world`, or `platform` for a model that describes the application rather than the world), an optional `icon` (a Lucide icon name, `map-pin` say), `schema` pointing at the `$id` above, `relationships` whose predicates are `Reference`-typed properties of the schema (dotted for nested ones, `participants.actor`) and whose targets are written into the generated schemas, so the property's `type` is fixed to the target's resource type and a pointer at the wrong one is refused, `validTime` naming the properties the record's in-world span is read from when it has any, and at least one `mapsTo` alignment. The validator refuses a predicate the schema does not have. Prefer schema.org names for properties where they exist.
