...
A `Generator<HistoryInput, HistoryOutput>` with id `history`. Input: a calendar, a species, a culture, and the realm itself as places, factions and titles, plus optional economy snapshots to seed prosperity, authored founders, canon events and parameter overrides. Output: people, relationships, events, tenures, claims, populations, economies and the consistency findings over them. The realm generator's output feeds it directly.
...
- **Conflict**. A law that prefers sons does not persuade the daughters it passes over: each keeps a `claim` to the title. A living claimant may press theirs, which declares a war between their house and the house holding the title. Each year of a war fights one battle, decided by a weighted roll on the two sides' strength, which is the people living in the lands each house and its vassals hold. The side that first wins two battles carries the war: the claimant is installed and the sitting holder deposed, or the claim is broken. Battles and the deposition hang off the war through `partOf`, so a chronicle can tell the war as one story.
