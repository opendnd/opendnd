...
Every world has a Map page that pans and zooms the way web maps do. The tiles drawn beneath the records come from the world itself. A world that says its map is its terrain gets them as SVG, rendered from its own coastlines when each tile is asked for, which is right at any depth — the coast keeps being a coast past the size anyone drew it at, because the roughness below that is generated from the coast itself and so is the same every time. A world with pictures somebody made gets those, at `/v1/worlds/{world}/tiles/{z}/{x}/{y}.png`; a world whose pictures live elsewhere on the web names a template under `map` instead, and a world that says its map comes from its terrain is drawn on the globe until there is terrain to draw. Over them sits every record of a model with a cell field, which the application finds in the schemas rather than knowing; today that is places and encounters. What is fetched follows the view, and it is two questions rather than one. The view is sampled; each sample is asked what is inside it, down to a level worth drawing at the zoom, with `?maxLevel=`; and each is asked what it is inside, with `?covers=`, because a county larger than the screen contains the view rather than sitting in it and no amount of looking inside will find it. so a zoomed-out map shows continents and kingdoms and a zoomed-in one the towns. A cell near a tile's size or bigger is drawn as its outline, a finer one as a mark, and each is labelled while there is room. Choosing one opens a short account of its record beside the map, with a way to the whole of it; the list beside the map does the same, and a search box finds a record by name and flies to it. A record with a cell has an On the map button that opens the map on it, and an editor gives a record that has none its place from here: Place on the map on the record, or its name in the search, then the spot on the map, at a cell as fine as the zoom or as chosen. The address follows the view, so a view can be shared.
