...
- **One `feature` for class features and species traits**, distinguished by `source`. **A trap is a `scene`, a familiar is a `relationship`, a vehicle is an `item`.**
...
- **Fields the server sets are marked `readOnly`**: `id`, `world`, `meta`, `module`, and `resourceType`, which every record carries so a body says what it is wherever it travels ([ADR-018](/adr/adr-018-fhir-shaped-resources/)). The API strips them from what a client sends and from the request-body schemas in its OpenAPI description, so a generated client is never asked for a revision it cannot know.
