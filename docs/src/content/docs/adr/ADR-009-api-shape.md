...
**Status:** Accepted, 2026-09-04. Implemented in `@opendnd/api`; tenancy is [ADR-011](/adr/adr-011-world-as-tenant/).
...
- **World scoping.** Every stored resource belongs to a world; the world comes from the path prefix `/v1/worlds/{world}/...` for stored data, never from the body alone. Anonymous generation needs no world. Authenticated users own worlds and grant roles on them. How that is enforced is [ADR-011](/adr/adr-011-world-as-tenant/).
...
- OpenAPI is generated from the same Zod schemas so the documentation cannot drift from the routes. `GET /v1/openapi.json` describes 42 paths and every model's schema, all of it derived from the ontology.
