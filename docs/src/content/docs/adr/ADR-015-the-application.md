...
## Decided later, 2026-09-08: a world is asked, not searched

- **Home is a question.** Inside a world the first thing on the page is a box to ask it something in plain words, with a few questions offered that name things actually on record, drawn from the world rather than written into the application. Searching has not gone: it sits in the header on every page, which is where a search belongs once it is wanted everywhere.
- **A question is answered where the conversation is.** A question put from a page is handed to the panel rather than answered where it was typed, so one conversation runs across the whole world instead of one per page. It waits until the deployment has said which models it holds, because a question sent before then is a question sent with no model.
- **The world is counted once.** How many records of each kind a world holds is one request for the whole world, not a page fetched per model, and the answer is drawn beside every model in the navigation, on the cards under Data, and on the home screen. A model the world holds nothing of is absent rather than zero, so a world can be counted without naming every model that exists.
- **Group labels are bare words.** A heading that names a group of navigation carries no icon of its own; icons belong to the things inside it.
