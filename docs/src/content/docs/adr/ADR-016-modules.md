...
- A module has events of its own, decided 2026-09-07: publishing, enabling, disabling and reordering each write to the outbox of the world it happened in, beside the writes, so a subscriber hears that content arrived. What it does not hear is each record the module brought; a subscriber that needs those reads the module.
- The stack can be reordered, nearest first, by an owner, decided 2026-09-07. A license check at enabling, and money, are not here.
